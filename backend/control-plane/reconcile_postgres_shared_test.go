...
func TestSharedDatabaseRendersDatabaseAndOwnerContract(t *testing.T) {
	...
	resources, scriptID, err := renderSharedPostgresResources(claim, testPostgresTarget(), postgresModeSharedDatabase, "SecretValue", false)
	if err != nil {
		t.Fatal(err)
	}
	if scriptID <= 1 {
...
func TestDatabaseAccessRendersScopedPrivileges(t *testing.T) {
	...
	resources, _, err := renderSharedPostgresResources(claim, testPostgresTarget(), postgresModeDatabaseAccess, "SecretValue", false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDatabaseAccessRejectsOwnerPrivilege(t *testing.T) {
	...
	if _, _, err := renderSharedPostgresResources(claim, testPostgresTarget(), postgresModeDatabaseAccess, "SecretValue", false); err == nil {
...
func TestManagedSQLStatusRequiresRequestedVersion(t *testing.T) {
	...
	ready, failed, _ := stackGresManagedSQLStatus(cluster, id, 2)
	if ready || failed {
		t.Fatalf("stale managed SQL version accepted: ready=%v failed=%v", ready, failed)
	}
	ready, failed, _ = stackGresManagedSQLStatus(cluster, id, 1)
	if !ready || failed {
