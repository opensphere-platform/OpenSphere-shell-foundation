...
@Component({
  ...
  template: `
    ...
    <section class="rm-work" *ngIf="active()==='monitoring'"><h2>Monitoring</h2><p>Collector 가용성, export queue와 backend 전송 상태를 실제 telemetry 계약으로 확인합니다.</p><dl class="os-kv"><dt>Lifecycle</dt><dd>{{svc.ready()?'Ready':'Pending'}}</dd><dt>Ready</dt><dd>{{svc.readyN()}}/{{svc.totalN()}}</dd><dt>OTLP</dt><dd>gRPC/HTTP</dd></dl></section>
