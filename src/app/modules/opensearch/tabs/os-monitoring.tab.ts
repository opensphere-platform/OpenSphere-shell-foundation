  ...
  template: `
    <section class="osm">
      ...
      <div class="osm-charts" *ngIf="metrics.state()==='ok' && metrics.series().timestamps.length">
        <article class="osm-chart-card"><div class="osm-chart-head"><div><h3>JVM heap & CPU</h3><p>노드 중 최대 사용률</p></div></div><os-uplot-line-chart [timestamps]="metrics.series().timestamps" [series]="percentSeries()" valueAxisTitle="Percent" ariaLabel="OpenSearch JVM heap과 CPU 사용률" /></article>
        <article class="osm-chart-card"><div class="osm-chart-head"><div><h3>Documents</h3><p>클러스터 문서 수</p></div></div><os-uplot-line-chart [timestamps]="metrics.series().timestamps" [series]="documentSeries()" valueAxisTitle="Documents" ariaLabel="OpenSearch 문서 수" /></article>
        <article class="osm-chart-card"><div class="osm-chart-head"><div><h3>Storage</h3><p>가용 디스크와 index store, GiB</p></div></div><os-uplot-line-chart [timestamps]="metrics.series().timestamps" [series]="storageSeries()" valueAxisTitle="GiB" ariaLabel="OpenSearch 디스크와 index store" /></article>
        <article class="osm-chart-card"><div class="osm-chart-head"><div><h3>Thread-pool rejections</h3><p>5분 구간 거부 작업 합계</p></div></div><os-uplot-line-chart [timestamps]="metrics.series().timestamps" [series]="rejectedSeries()" valueAxisTitle="Rejected / 5m" ariaLabel="OpenSearch thread pool 거부 작업" /></article>
      </div>
      <section class="osm-nodes" aria-labelledby="osm-node-title">
        ...
        <div class="osm-node-grid" [style.--node-columns]="nodeColumns()">
          <article class="osm-node" *ngFor="let node of metrics.nodes(); trackBy: trackNode" [attr.data-node]="node.node">
            <div class="osm-node-head"><div><span class="osm-node-kicker">OpenSearch node</span><h3>{{node.node}}</h3></div><span class="label" [ngClass]="node.joined && node.metricsObserved?'label-success':'label-warning'">{{node.joined && node.metricsObserved?'live':node.joined?'metrics pending':'join pending'}}</span></div>
