...
import { OsService } from '../opensearch/os.service';
import { OsOverviewTab } from '../opensearch/tabs/os-overview.tab';
import { OsNodesTab } from '../opensearch/tabs/os-nodes.tab';
import { OsIndicesTab } from '../opensearch/tabs/os-indices.tab';
import { OsShardsTab } from '../opensearch/tabs/os-shards.tab';
...
@Component({
  ...
  template: `
    ...
    <section class="de-work de-work--flush" *ngIf="tab()==='monitoring' && engine==='opensearch'">
      <os-monitoring />
    </section>
    ...
    <section class="de-work" *ngIf="tab()==='domain'">
      <ng-container *ngIf="engine==='opensearch'; else genericDomain">
        <div class="de-section-head"><div><span class="vl-eyebrow">Read-only service telemetry</span><h2>OpenSearch data views</h2></div><button class="btn btn-sm" type="button" (click)="os.refresh()" [disabled]="os.busy()"><os-cicon [icon]="iRenew" [size]="16"/> 새로고침</button></div>
        <p>OpenSearch read-only proxy가 cluster health, node, index, shard, template와 task 상태를 15초 주기로 수집합니다. 마지막 동기화: {{os.lastSync() || '확인 중'}}</p>
        <nav class="de-subtabs" aria-label="OpenSearch data views">
          <button type="button" [class.active]="osView()==='overview'" [attr.aria-current]="osView()==='overview'?'page':null" (click)="openOsView('overview')">Cluster</button>
          <button type="button" [class.active]="osView()==='nodes'" [attr.aria-current]="osView()==='nodes'?'page':null" (click)="openOsView('nodes')">Nodes</button>
          <button type="button" [class.active]="osView()==='indices'" [attr.aria-current]="osView()==='indices'?'page':null" (click)="openOsView('indices')">Indices</button>
          <button type="button" [class.active]="osView()==='shards'" [attr.aria-current]="osView()==='shards'?'page':null" (click)="openOsView('shards')">Shards</button>
          <button type="button" [class.active]="osView()==='templates'" [attr.aria-current]="osView()==='templates'?'page':null" (click)="openOsView('templates')">Templates & aliases</button>
          <button type="button" [class.active]="osView()==='tasks'" [attr.aria-current]="osView()==='tasks'?'page':null" (click)="openOsView('tasks')">Tasks & settings</button>
        </nav>
        <os-overview *ngIf="osView()==='overview'" (jump)="openOsView($event)" />
        <os-nodes *ngIf="osView()==='nodes'" />
        <os-indices *ngIf="osView()==='indices'" />
        <os-shards *ngIf="osView()==='shards'" />
        <os-templates *ngIf="osView()==='templates'" />
        <os-tasks *ngIf="osView()==='tasks'" />
      </ng-container>
      <ng-template #genericDomain><h2>{{domainLabel()}}</h2><p>{{spec.name}} 고유 데이터·접근 영역입니다. 운영 객체는 서비스 endpoint와 SecretRef를 통해서만 소비합니다.</p><dl class="de-kv"><dt>Service endpoint</dt><dd class="os-mono">{{spec.endpoint}}:{{spec.port}}</dd><dt>Credential source</dt><dd class="os-mono">{{form().authSecret||'Operator generated Secret'}}</dd><dt>Namespace</dt><dd class="os-mono">{{spec.namespace}}</dd><dt>Managed resource</dt><dd>{{runtimePhase()}}</dd></dl></ng-template>
    </section>
...
export class DataEnginePluginComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnInit():void{this.hydrateRouteTab();this.runtime.start();if(this.engine==='opensearch'){this.os.start();this.osMetrics.start();}void this.initialize();}
  ngOnChanges(ch:SimpleChanges):void{const change=ch['engine'];if(change&&!change.firstChange){if(change.previousValue==='opensearch'){this.os.stop();this.osMetrics.stop();}this.form.set(defaultForm(this.spec));this.tab.set('overview');this.hydrateRouteTab();this.osView.set('overview');if(this.engine==='opensearch'){this.os.start();this.osMetrics.start();}void this.initialize();}}
  ...
  private hydrateRouteTab():void{const wanted=this.vr.tab() as Tab;if(this.tabs.some(item=>item.id===wanted))this.tab.set(wanted);}
