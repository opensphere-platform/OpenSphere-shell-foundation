  ...
  template: `
    ...
    <ng-container *ngIf="tab()==='overview'">
      <section class="pgp-steps" aria-label="Keycloak plugin 설치 단계">
        <button class="pgp-step" [class.done]="controlPlaneReady()" [class.current]="!controlPlaneReady()" (click)="openTab('dependency')"><span class="pgp-step-n">1</span><span><b>실행 기반 준비</b><small>Foundation control-plane</small></span></button>
        <button class="pgp-step" [class.done]="exists()" [class.current]="controlPlaneReady()&&!exists()" (click)="openTab('plan')"><span class="pgp-step-n">2</span><span><b>서비스 구성</b><small>버전·리소스·접근 정책</small></span></button>
    ...
    <section class="pgp-workspace" *ngIf="tab()==='dependency'">
    ...
    <section class="pgp-workspace" *ngIf="tab()==='plan'">
    ...
    <section class="pgp-workspace" *ngIf="tab()==='upgrade'">
      ...
      <button class="btn btn-primary" type="button" (click)="openTab('plan')">설치·운영 구성 검토</button>
    </section>
...
export class KeycloakComponent implements OnInit, OnDestroy {
  ...
  readonly tabs: {id:Tab;label:string;runtime?:boolean}[]=[{id:'overview',label:'Overview'},{id:'dependency',label:'실행 기반'},{id:'plan',label:'설치·운영 구성'},{id:'topology',label:'Topology',runtime:true},{id:'consumers',label:'Consumers'},{id:'protection',label:'Backup & Security'},{id:'events',label:'Events',runtime:true},{id:'upgrade',label:'Upgrade'},{id:'documentation',label:'Documentation'}];
