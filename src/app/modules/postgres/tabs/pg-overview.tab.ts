...
@Component({
  ...
  template: `
    ...
    <div class="cards">
      <div class="card">
        <div class="card-h">클러스터 · {{ svc.name }}</div>
        <dl class="kv">
      ...
      <div class="card">
        ...
        <dl class="kv">
          <dt>쓰기(RW)</dt><dd class="mono">{{ svc.name }}-rw.{{ svc.ns }}.svc:5432</dd>
          <dt>읽기(RO)</dt><dd class="mono">{{ svc.name }}-ro.{{ svc.ns }}.svc:5432</dd>
          <dt>자격 Secret</dt><dd class="mono">{{ svc.name }}-app · pgc-&lt;claim&gt;-conn</dd>
        </dl>
        <p class="muted" style="margin:.5rem 0 0">키: host·port·dbname·user·password·uri. 값은 정책상 비노출 — <code>kubectl get secret</code>.</p>
      </div>
    </div>

    <div class="sec-h">상태 조건 (conditions)</div>
