  ...
  template: `
    ...
    <section *ngIf="tab()==='cluster'" class="pgp-workspace">
      ...
      <form class="pgp-form" (ngSubmit)="apply()">
        ...
        <fieldset [disabled]="applying()"><legend>Security & credentials</legend><div class="pgp-form-grid"><label><span>Authentication Secret</span><input name="authSecret" [ngModel]="form().authSecret" disabled/><small>임의 Secret 이름은 허용하지 않습니다.</small></label><label><span>Secret 작업 사유</span><input name="secretReason" [(ngModel)]="secretReason" placeholder="8자 이상 생성 또는 회전 사유"/></label><div class="rf-actions"><button type="button" class="btn btn-sm" (click)="createCredential()" [disabled]="secretReason.trim().length<8"><os-cicon [icon]="iPassword" [size]="16"/> Secret 생성/회전</button></div><label><span>Monitoring</span><input value="Exporter 미구성" disabled/><small>지원되지 않는 설정은 활성화하지 않습니다.</small></label></div><div class="rf-once" *ngIf="rf.oneTimeCredential() as c"><b>한 번만 표시되는 RustFS 자격</b><div class="rf-mono">Access key: {{c.accessKey}}</div><div class="rf-mono">Secret key: {{c.secretKey}}</div></div></fieldset>
    ...
    <section *ngIf="tab()==='documentation'" class="rf-work"><div class="pgp-section-head"><div><span class="vl-eyebrow">Console Manual Registry</span><h2>Documentation</h2></div></div><dl class="os-kv"><dt>문서 ID</dt><dd class="rf-mono">plugin:foundation/rustfs-operations-ko</dd><dt>화면 경로</dt><dd class="rf-mono">/p/foundation/rustfs</dd><dt>관리 API</dt><dd>ListBuckets · CreateBucket · DeleteBucket allowlist</dd></dl><a class="btn btn-sm btn-primary" [href]="manualUrl">한글 안내서</a><a class="btn btn-sm" [href]="SPEC.docs" target="_blank" rel="noreferrer">RustFS 공식 문서</a></section>
  `,
})
