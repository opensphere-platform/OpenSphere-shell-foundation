...
test('PostgreSQL restores the detailed Overview and Prometheus monitoring workspaces', () => {
  ...
  assert.match(monitoring, /\.pg-monitoring \{ container-type: inline-size;/);
  assert.match(monitoring, /@container \(max-width: 75rem\) \{ \.pg-monitoring-metrics \{ grid-template-columns: repeat\(4, minmax\(0, 1fr\)\)/);
